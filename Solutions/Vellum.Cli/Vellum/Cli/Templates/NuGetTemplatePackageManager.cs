...
public class NuGetTemplatePackageManager : ITemplatePackageManager
{
    ...
    private async Task EnrichTemplatePackage(TemplatePackage templatePackage)
    {
        ...
        {
            string filePath =
                System.IO.Path.GetFullPath(System.IO.Path.Combine(templatePackage.InstallationPath, template.NestedFilePath));
            ...
            if (yamlBlock != null)
            {
                string yaml = string.Join(System.Environment.NewLine, yamlBlock.Lines.Lines.Select(stringLine => stringLine.ToString())
